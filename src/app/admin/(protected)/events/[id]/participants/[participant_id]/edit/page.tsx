...
export default function EditParticipantPage({ params }: { params: { id: string, participant_id: string } }) {
  ...
  if (fetching) return <div className="p-10 flex h-screen items-center justify-center"><div className="animate-pulse text-primary font-semibold font-display">Memuat Data Peserta...</div></div>

  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-10">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold tracking-tight text-slate-900 font-display">Edit Peserta</h1>
        <Link href={`/admin/events/${params.id}`}>
          <Button variant="outline" className="border-emerald-300 text-primary hover:bg-emerald-50 rounded-xl">Batal</Button>
        </Link>
      </div>

      <Card className="glass shadow-sm border-t-4 border-t-primary rounded-2xl">
        <form onSubmit={handleSubmit}>
          <CardHeader>
            <CardTitle className="text-xl font-display">Data Diri Peserta</CardTitle>
          </CardHeader>
          ...
          <CardFooter className="bg-slate-50/50 p-6 rounded-b-2xl border-t border-slate-100 mt-4">
            <Button type="submit" disabled={loading} className="w-full h-12 text-base font-bold bg-primary hover:bg-primary-container text-on-primary shadow-md rounded-xl font-display transition-all">
