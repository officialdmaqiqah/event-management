  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      ...
      <Card className="shadow-sm rounded-2xl">
        <CardHeader className="bg-slate-50 border-b border-slate-100">
          <CardTitle className="font-display">2. Konfigurasi per Hari</CardTitle>
              ...
              return (
                <div key={day.id} className={`p-4 sm:p-6 transition-colors ${config.enabled ? 'bg-white' : 'bg-slate-50/50'}`}>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    />
                    <Label htmlFor={`day-${day.id}`} className={`text-lg font-bold cursor-pointer font-display ${config.enabled ? 'text-primary' : 'text-slate-400'}`}>
                      Hari {day.name}
                    </Label>
        ...
        <CardFooter className="bg-slate-50 border-t border-slate-100 p-6 flex flex-col sm:flex-row gap-4">
          ...
          >
            <Save className="w-5 h-5 mr-2 text-primary" /> Simpan Draft Sementara
          </Button>
          <Button 
            onClick={handleGenerate} 
            disabled={loading}
            className="w-full sm:w-2/3 h-12 text-base font-bold bg-primary hover:bg-primary-container text-on-primary shadow-md rounded-xl font-display"
          >
